...
test("public feed follows the panel grid and leaves admin separate", () => {
  ...
  assert.match(dashboard, /useSWR<DashboardPayload>\(\s*"\/api\/dashboard"/);
  assert.doesNotMatch(dashboard, /isMounted && isConnected \? "\/api\/dashboard" : null/);
