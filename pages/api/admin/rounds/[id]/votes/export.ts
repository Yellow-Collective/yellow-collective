...
export default async function handler(
  ...
  try {
    ...
    const csv = createAdminRoundVotesCsv({ round, votes });

    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${getAdminRoundVotesCsvFilename(round)}"`
    );
    return res.status(200).send(csv);
  } catch (error) {
    console.error("Admin round votes export failed", error);
    return res.status(500).json({ error: "Unable to export round votes." });
  }
}
